...
export type IFormlidField<TFormValue extends object> = <TName extends UKey<TFormValue>>(
  name: TName
) => {
  name: TName;
  value: TFormValue[TName];
  oninput: (e: CustomInputEvent) => void;
  onfocus: (e: CustomInputEvent) => void;
  onblur: (e: CustomInputEvent) => void;
};

export type IFormlidMeta<TFormValue extends object> = <TName extends UKey<TFormValue>>(
  name: TName
) => {
  error: string;
  isError: boolean;
...
export type IFormlidExtension<TFormValue extends object> = <TName extends UKey<TFormValue>>(
  name: TName
...
export interface IFormlidHelpers<TFormValue> {
...
type validationSchema<TFormValue> = {
  [key in keyof TFormValue]: Yup.Schema<TFormValue[key]>;
};

// Formlid의 Props
interface FormlidProps<TFormValue extends object> {
  initialValues: TFormValue;
  validationSchema?: validationSchema<TFormValue>;
  onsubmit: (formData: TFormValue, helpers: IFormlidHelpers<TFormValue>) => void;
 ...
 * @returns
 */
const Formlid = <TFormValue extends object>(props: FormlidProps<TFormValue>) => {
  // Form methods
  ...
  const validate = async (formData: TFormValue) => {
    if (props.validationSchema == null) return true;
    let success = (await Promise.all(keys.map(validateField))).every(Boolean);
    return success;
  };
   ...
   * @returns
   */
  const field = <TName extends UKey<TFormValue>>(name: TName) => ({
    name,
  ...
  // helpers
  const helpers: IFormlidHelpers<TFormValue> = {
    isSubmitted,
    isSubmitting,
    getValues,
    setValue,
  ...
  const FormlidProvider: Component<{ children: JSX.Element }> = (props) => {
    return (
